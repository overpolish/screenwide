...
export function EditorWindow() {
  ...
  const kind = currentEditorKind() ?? "recording";
  ...
  const editState = useMemo<EditorEditState>(
    () => ({
      audioTrackVolumes,
      bakeCamera,
      cameraCompression,
      ...
      <EditorPanel
        ...
        onMinimize={() => {
          getCurrentWindow()
            .minimize()
            .catch((cause: unknown) => {
              console.error("Could not minimize the editor window", cause);
            });
        }}
        ...
        onToggleMaximize={() => {
          getCurrentWindow()
            ...
            .catch((cause: unknown) => {
              console.error(
                "Could not maximize or restore the editor window",
                cause,
              );
            });
        }}
