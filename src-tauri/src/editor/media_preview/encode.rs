...
#[path = "encode/process.rs"]
mod process;
#[cfg(test)]
pub(super) use process::progress_milliseconds;
pub(in crate::editor) use process::run_export;
...
pub(in crate::editor) use timeline_args::timeline_audio_mapping_args;
...
pub(in crate::editor) fn remux_temp_path(destination: &Path) -> PathBuf {
...
pub(in crate::editor) fn remux_error(stderr: &[u8]) -> String {
...
/// the machine, and because it is asked every time the editor window is told
