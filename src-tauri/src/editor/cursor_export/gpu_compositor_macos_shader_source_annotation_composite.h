...
/// The pass that draws a layer's marks, whatever shape each one is. It comes
/// after every shape's own source: each kind is a branch here and a layer
...
static float4 composite_annotations(
  ...
  for (uint index = 0; index < count; ++index) {
    ...
    if (mark.above_camera != above_camera) continue;
