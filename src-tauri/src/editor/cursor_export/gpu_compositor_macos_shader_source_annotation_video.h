...
static float4 annotation_video_pixel(const device AnnotationUniforms *marks,
    ...
    uint2 atlas) {
  float4 value = composite_annotations(float4(0), marks, count, above, point,
...
kernel void overlay_annotation_luma(
    ...
    const device AnnotationUniforms *marks [[buffer(12)]],
  ...
  float4 rgba = annotation_video_pixel(marks, count, above, float2(gid) + 0.5, canvas, samples, numbers, atlas);
...
kernel void overlay_annotation_chroma(
  ...
  for (uint y = 0; y < 2; y++) for (uint x = 0; x < 2; x++) {
    float4 rgba = annotation_video_pixel(marks, count, above, float2(gid * 2u + uint2(x,y)) + 0.5, canvas, samples, numbers, atlas);
