...
kernel void overlay_annotation_luma(
  ...
  float4 rgba = annotation_video_pixel(marks, count, above, float2(gid) + 0.5, canvas, samples);
...
kernel void overlay_annotation_chroma(
  ...
  for (uint y = 0; y < 2; y++) for (uint x = 0; x < 2; x++) {
    float4 rgba = annotation_video_pixel(marks, count, above, float2(gid * 2u + uint2(x,y)) + 0.5, canvas, samples);
