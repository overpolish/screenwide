...
- (void)rightMouseDown:(NSEvent *)event {
  if (self.surface.pointerDownCallback)
    self.surface.pointerDownCallback(self.surface.pointerDownContext);
  if (self.surface.webview != nil)
    [self.window makeFirstResponder:self.surface.webview];
  [self reportContextMenuAtPoint:[self convertPoint:event.locationInWindow
                                           fromView:nil]];
}
- (void)mouseDown:(NSEvent *)event {
  ...
  // AppKit delivers Control-click as a left press, so the second way to the
  // layer menu is answered here rather than in `rightMouseDown:`.
  if (event.buttonNumber == 0 && event.clickCount <= 1 &&
      (event.modifierFlags & NSEventModifierFlagControl) != 0 &&
      [self reportContextMenuAtPoint:point])
    return;
...
- (void)mouseDragged:(NSEvent *)event {
  ...
  if (self.selectionDragActive) {
    ...
    if (self.selectionDragOperation == 5) {
      ...
      double vectorX = handleX - anchorX, vectorY = handleY - anchorY;
      ...
      if (snapping)
        scale = snap_selection_resize(
            ...
            edges, pane, effectiveMinimumScale, maximumScale);
      else
        clear_selection_snap_guides(self.surface);
      x = anchorX + (start.x - anchorX) * scale; y = anchorY + (start.y - anchorY) * scale;
      ...
      if (self.selectionMoveAutoFitActive && !optionHeld) {
        ...
        // Rebase the mouse-down targets so Option can grow the canvas again.
