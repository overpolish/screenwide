  ...
  *settings
  ...
  if !panes.is_empty() {
    ...
    let workspace_panes = panes
      ...
      .collect::<Vec<_>>();
    manager.workspace_scene = Some(crate::editor::preview_workspace_model::recording_scene(
      WorldRect {
        x: viewport.x,
        y: viewport.y,
  ...
  // The decoder path keeps the batch closed unless a fit reset must be folded
  // into this layout; its present arrives later and finds the geometry parked.
  #[cfg(target_os = "macos")]
