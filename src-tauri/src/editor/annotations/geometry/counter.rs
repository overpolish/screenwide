...
/// with its curve: `a` is the disc's centre and `b` the centre of the small
/// circle the tail ends in, `rounding` is the disc's radius, `low` the tip's
...
/// the tail ends in. The sides are the tangents joining them, so the outline
/// arrives at the tip without ever coming to a point.
pub(crate) fn prepare_counter(
  ...
  let reach = radius * COUNTER_TAIL_REACH - result.low;
...
pub(crate) const COUNTER_TAIL_REACH: f32 = 1.85;
const COUNTER_TIP_SHARE: f32 = 0.17;
