...
/// A region smaller than the overlay still centres it, so the ring stays over
/// the thing being captured rather than jumping to a corner.
#[test]
fn centres_a_region_smaller_than_the_overlay() {
  let origin = centred_origin(50.0, 40.0);
